...
function updateCounts() {
    Object.keys(SUBCATS).forEach(s => {
        const el = $('count-' + s);
        if (el) el.textContent = ERRORS.filter(e => e.subcat === s).length;
    });
}
...
function getFiltered() {
    ...
    if (searchQuery) {
        ...
        if (searchFilter !== 'all') {
            list = list.filter(e => e.cat === searchFilter || e.subcat === searchFilter);
        }
    } else {
        list = list.filter(e => e.subcat === currentSubcat);
        if (currentSeverity !== 'all') list = list.filter(e => e.severity === currentSeverity);
    }
    return list;
}
...
globalSearch.addEventListener('input', function () {
    ...
    searchTimer = setTimeout(function () {
        ...
        if (searchQuery) {
            ...
            searchResultsInfo.textContent = 'Found ' + getFiltered().length + ' results for "' + searchQuery + '"';
        } else {
